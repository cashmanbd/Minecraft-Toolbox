package com.matlabworld.j2blocks;
...
public class WorldTest {
    ...
    public static void test(double[][] data) throws IOException {
...
// height. On top of the glass structure we put a layer of grass.
        for(int x = 0; x < data.length; x++) {
            for(int z = 0; z < data[0].length; z++) {
                // Set glass
                for(int y = 0; y < 50; y++) {
                    if (y < data[x][z] && y > (data[x][z] - 2) ) {
                        world.setBlock(x, y, z, SimpleBlock.GRASS);
                    }
                }
